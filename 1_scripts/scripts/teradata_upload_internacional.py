    ...
    try:
        query = """
                    SELECT
                        ...
                        "Settlement Gross Charge - USD" AS SETTLEMENT_GROSS_CHARGE_USD
                    FROM read_csv_auto(
                        ?,
                        delim=';',
                        encoding = 'utf-16',
...
def main() -> None:
    ...
    TABLE_COLUMNS = OrderedDict(
        ...
        SETTLEMENT_GROSS_CHARGE_USD=NUMBER(precision=15, scale=5),
        DIRECTION=CHAR(3),
        ...
        for file in paths:
            if str(file).endswith(".csv"):
                ...
                df = sanitize_numeric_col(
                    df,
                    ["CHARGED_MINUTES", "CHARGED_MB", "SETTLEMENT_GROSS_CHARGE_USD"],
                    is_integer=False,
                )
                df = sanitize_numeric_col(
